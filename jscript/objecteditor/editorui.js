...
function update_point_data() { // передаётся объект целиком
	...
	$("#m_lat").val(lat);
	$("#m_lon").val(lon);
	$("#m_lat_d").html(lat.toFixed(4) + ',');
	$("#m_lon_d").html(lon.toFixed(4));
...
$(".allowDrag").click(function(){
	if ($(this).hasClass("active")) {
		e_objects.options.set({ draggable: 0 });
		return true;
	}
	e_objects.options.set({ draggable: 1 });
});
...
$("#type").change(function () {
	var ref      = parseInt($("#type option:selected").attr('ref'), 10),
		newStyle = $("#type option:selected").attr('apply'),
		desc     = $("#type option:selected").text(),
		attr     = $("#type option:selected").html(),
		newType  = $(this).val();
	if (ref !== prop.pr) {
	...
	prop.pr         = ref;
	prop.decription = desc;
	prop.attr       = normalize_style(newStyle);
	prop.type       = newType;
	$("#description").html(attr);
	$('#l_attr option[value="' + newStyle + '"]').prop('selected', true);
...
$("#l_addr").keyup(function(){
	var string = $(this).val();
	if (string.length < 4) {
		return false;
	}
	findSimilarAddresses(string);
});
