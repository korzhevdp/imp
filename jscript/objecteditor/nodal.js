	...
	$(".map_calc").unbind().click(function () {
		...
		$.ajax({
			...
			success  : function () {
				var a;
				for (a in DCdata) {
					if (DCdata.hasOwnProperty(a)) {
						nePolygons.removeAll();
						checkPointsInclusion(DCdata[a].ym, a);
						nePolygons.removeAll();
					}
				}
			},
...
function checkPointsInclusion(data, id) {
	...
	if (prop.pr === 1) {
		if (nePolygons.get(0).geometry.contains(src_geometry)) {
	...
	if (prop.pr === 2) {
		for (c in src_geometry) {
			if (src_geometry.hasOwnProperty(c)) {
				...
				if (nePolygons.get(0).geometry.contains(src_geometry[c])) {
					console.log("Line");
					$("#p" + id).prop('checked', true);
					return true;
				}
			...
			if (src_geometry.hasOwnProperty(c)) {
				for (d in src_geometry[c]) {
					if (src_geometry[c].hasOwnProperty(d)) {
						...
						if (nePolygons.get(0).geometry.contains(src_geometry[c][d])) {
							console.log("Poly");
							$("#p" + id).prop('checked', true);
							return true;
						}
...
function calculateToleranceParameters(digits) {
	...
	var startPoint = prop.map_center,
		endPoint   = [
			prop.map_center[0] + (1 / Math.pow(10, digits)),
			prop.map_center[1] + (1 / Math.pow(10, digits))
		];
...
function findSimilarAddresses(string) {
	$.ajax({
		url      : "/nodal/similaraddress",
		type     : "POST",
		data     : {
			str  : string
		},
