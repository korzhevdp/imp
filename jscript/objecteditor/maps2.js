...
function make_environment(object) {
	...
	if (prop.pr === 1) {
		...
		if(    (prop.pr === 2 && object.geometry.getCoordinates().length > 1)
			|| (prop.pr === 3 && object.geometry.getCoordinates()[0].length > 2)) {
			//alert(object.geometry.getCoordinates()[0].length);
...
$(".editorTable").height($(window).height() - 44 + 'px');
$("#YMapsID").height($(window).height() - 100 + 'px');
ymaps.ready(init);
