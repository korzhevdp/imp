...
function init_nav() {
	...
	map = new ymaps.Map("YMapsID",
		{center: config.mcenter, zoom: config.initZoom, type: config.layerTypes[0].label, behaviors: ["default","scrollZoom","drag"]},
		{projection: config.proj, maxZoom: config.maxZoom, minZoom: config.minZoom, checkZoomRange : false, suppressMapOpenBlock: true, yandexMapDisablePoiInteractivity : true, yandexMapAutoSwitch: false});
	map.controls.add(typeSelector).add("mapTools");
		...
		function cycle_objects() {
			...
			function centerOnObject() {
				...
				if (demoCounter < len) {
					...
					distance = ymaps.coordSystem.geo.getDistance(map.getCenter(), fx[gt](object));
					if (distance < 2500){
						if (gt === "Point") {
					...
					function openB() {
						...
						closeTimer = setTimeout(closeB, 12000);
						return false;
					};
				...
				lilCycle = setTimeout(centerOnObject, 20000);
			}
			demoCounter = 0;
			centerOnObject();
			if (isDemoStopped) {
	...
	*/
	if (config.hasNav === 1 || config.hasNav) {
		$.ajax({
			type     : "POST",
